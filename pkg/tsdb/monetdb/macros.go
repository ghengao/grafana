...
type MonetDbMacroEngine struct {
	TimeRange *tsdb.TimeRange
}

func NewMonetDbMacroEngine() tsdb.SqlMacroEngine {
	return &MonetDbMacroEngine{}
}

func (m *MonetDbMacroEngine) Interpolate(timeRange *tsdb.TimeRange, sql string) (string, error) {
...
func (m *MonetDbMacroEngine) evaluateMacro(name string, args []string) (string, error) {
	format := time.RFC3339
	switch name {
	case "__time":
		if len(args) == 0 {
